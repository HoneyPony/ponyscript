...
impl TypeName {
    ...
    pub fn to_raw_name(&self) -> String {
        match &self {
            TypeName::Primitive(what) => {
                // Node types are ALL represented as pointers... TODO support value types...? Will be
                // annoying!
                format!("struct Raw{}*", what)
            }
            TypeName::Void => {
                "void".to_string()
            }
    ...
    pub fn to_struct_name(&self) -> String {
        match &self {
            TypeName::Primitive(what) => {
                return format!("Raw{}", what);
            }
            _ => { return "NOT ALLOWED".to_string(); }
        }
    }
}
